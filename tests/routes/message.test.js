...
import { UserRouter, SessionRouter, ChatRouter, MessageRouter } from '../../routes/index.js'
...
describe('Test message route', () => { 
    ...
    it('Get all the messages of a chat', async() => {
        ...
        const createChatRes = await api.post('/api/chats')
            .set('authorization', "Bearer " + tokenMock.token)
            .send({
                userId: user._id,
                message: "Hello there!"
    ...
    it('Creates a new message in a chat', async() => {
        ...
        const createMessage = await api
            .post(`/api/messages?chatId=${chatId}`)
            .set('authorization', 'Bearer ' + tokenMock.token)
            .send({
                message: 'Hello!'
            })
